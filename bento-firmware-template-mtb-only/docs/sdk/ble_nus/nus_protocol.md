...
Feed RX bytes from the NUS RX characteristic. May deliver one full frame, a partial frame, or multiple concatenated frames. Non-blocking.
...
Called from ble_nus when Approve/Deny button pressed. decision_approve: true = "once", false = "deny". Emits the permission JSON frame on NUS TX with the stored prompt id.
...
Called from ble_nus BTM_ENCRYPTION_STATUS_EVT handler. When true, status ack's data.sec field reports "sec":true.
...
Passive watchdog tick — call periodically (e.g., from UI render at ~10 Hz). Emits BUDDY_UI_STATE=SLEEP via IPC if 30 s have passed without a parseable frame and the link is still considered connected.

## Constants

| Name | Value |
|---|---|
| `NUS_PROTOCOL_H` | `#include` |
