...
This is the **mtb-only** variant: plain C on FreeRTOS, no MicroPython.
No REPL, no /main.py, and the BENTO IDE cannot flash it — liveness is
the `[HB]` heartbeat on the UART every 10 s. Config and WiFi
credentials persist through the C storage layer (`storage_c/`),
byte-compatible with the mtb-mpy variant on the same board.

It boots to a touch Home screen with ten menus, reads
every sensor on the board, and talks to the TESAIoT cloud. All of that is here
as source you can read and change — except five areas that arrive as prebuilt
libraries. Which five, and why, is spelled out below.
...
| `menus` | all ten menus, whether each is on, and its size on disk |
...
```
_core         animation     bentoclaw     edge_ai       environ       gpio_rgb
hsm           joystick      motion        smart_watch   wifi_connect
```
...
There is no VM in this variant. Your code is ordinary FreeRTOS
tasks on CM33_NS and LVGL pages on CM55 — see section 4, and the
C storage API in `bento_libs/claw/common/storage_c/bento_storage.h`.

## 6. What is prebuilt, and why

Five areas ship as static libraries in `lib/` rather than as source. Their
...
carry the platform, which is far too large to ship inside a template — nothing here needs the MicroPython port.
...
lib/                       the five prebuilt areas, with headers and signature
```
