...
/* Proves that the private key in key_oid belongs to the certificate in cert_oid,
 * by signing a challenge in the chip and verifying it against the certificate's
 * public key. 1 = match, 0 = mismatch, negative = could not run.
 * Not used by the connect path — reachable as optiga.verify_pair(). */
int optiga_verify_cert_key_pair(uint16_t cert_oid, uint16_t key_oid);

/* Put a data object's change access condition back to "an ordinary write is
 * accepted", clearing the Int(anchor) that Protected Update set. Reversible by
 ...
 *
 * manifest_anchor_oid is non-zero once Protected Update has locked the object:
 * the Change access condition reads Int(OID) and a plain write is refused from
 * then on. payload_version is the chip's anti-rollback counter — the next
 * manifest must carry a strictly greater value.
 */
typedef struct {
    uint16_t oid;
    bool     valid;
    ...
    uint8_t  data_type;            /**< tag E8; 0x11 trust anchor, 0x12 device cert */
} optiga_slot_info_t;
...
/* Opens the OPTIGA application, recovering from one left open by a previous
 * host session. Returns false when the secure element is unusable. */
bool optiga_trust_open_application(void);

/* Releases one reference taken above; the application closes only when the last
 * owner lets go. */
...
/* Verify a store-signed Edge AI model blob, in the secure element.
 *
 * Returns 1 when the signature is good; negative otherwise, and the codes are
 * distinct on purpose -- "no signature", "malformed", "the chip said no" and
 * "could not ask the chip" call for different responses, and collapsing them
 * would make a bus failure look like an attack. See ipc_model_stage_defs.h. */
...
/* Where write_device_certificate_and_verify() puts the certificate: Cert 2,
 * the slot this project's pre-provisioning plan pairs with ECC Key 2 (0xE0F1).
 * Defined here as well as in the .c so callers can verify the pair afterwards
 * without hardcoding the number a second time. */
