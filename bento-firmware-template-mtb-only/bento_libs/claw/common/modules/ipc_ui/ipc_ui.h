...
/**
 * Project extension dispatch hook for UI-band IPC opcodes not handled by the
 * built-in switch (weak default returns false / no-op). Called from
 * process_ui_command() in GFX-task context, so implementations may touch the
 * display / GFX resources (e.g. the QWA309 DFR0522 RGB matrix on display I2C).
 *
 * @param cmd   The IPC opcode (IPC_CMD_UI_* range).
 * @param data  Pointer to the raw 128-byte IPC payload (ipc_msg_t.data).
