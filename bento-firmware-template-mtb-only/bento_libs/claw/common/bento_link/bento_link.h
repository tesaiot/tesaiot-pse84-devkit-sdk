...
/* Capability flags */
#define BL_F_LOSSY      (1u << 0)  /* frames may drop (no retransmit)          */
#define BL_F_BULK_RING  (1u << 1)  /* supports negotiated shared-memory bulk   */
...
struct bento_link_s {
    ...
    bool (*send_data)(bento_link_t *self, uint8_t stream_id,
                      const void *buf, uint16_t len);

    /* Receive callbacks. Backends MUST invoke these OFF interrupt context
     * (each backend trampolines: ISR -> queue -> delivery task). */
    void (*register_receive)(bento_link_t *self,
                             void (*ctrl_cb)(uint8_t cmd, uint32_t value),
                             void (*data_cb)(uint8_t stream_id, const void *buf,
                                             uint16_t len, uint16_t seq));

    /* Capability advertisement — callers adapt, never assume. */
    const char *name;          /* "ipc" | "usb" | "ble" | "wifi"              */
    uint16_t    mtu;           /* max data-plane payload per frame            */
    uint32_t    bandwidth_bps; /* honest sustained budget for the data plane  */
    uint8_t     flags;         /* BL_F_*                                      */
};
...
/** Register a backend (called once from each backend's init). */
