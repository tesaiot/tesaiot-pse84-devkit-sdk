 ...
 *              the desktop -> device command dispatcher in nus_commands.c.
 *
 ...
 *              notifications (SPEC §7.5). nus_commands.c pushes ids when a
 *              bento.notify.show lands; CM55 LCD taps drain them back via
 *              nus_events_drain_pending_ack().
 *
 *              Reference: TESAIoT_PLAN/2026-4/Bento_Buddy/SPEC.md §5.2.
 *
 ******************************************************************************/
 ...
 * nus_commands.c:cmd_bento_notify_show. Dedups; evicts oldest when full. */
void nus_events_push_pending_ack(const char *id);

/* Drain an id from the pending-ack FIFO. Returns true if found (and removed),
 * false otherwise. Called from ipc_bento_buddy_bridge.c when an
