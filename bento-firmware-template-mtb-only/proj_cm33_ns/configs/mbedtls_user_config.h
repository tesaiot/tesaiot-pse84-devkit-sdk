...
//#define MBEDTLS_ECP_DP_SECP256R1_ENABLED  /* kept from platform config */
#define MBEDTLS_ECP_DP_SECP384R1_ENABLED    /* required: E7 CA uses ECDSA P-384 */
...
/* Enable ECDHE-ECDSA cipher suites for HTTPS to api.tesaiot.com
 * (Let's Encrypt E7 uses ECDSA P-384, leaf cert uses ECDSA P-256).
 * Dependencies already enabled in pse84_mbedtls_config.h:
 * MBEDTLS_ECDH_C, MBEDTLS_ECDSA_C, MBEDTLS_ECP_DP_SECP256R1/384R1.
 */
#define MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED

/* Enable ECDHE-RSA cipher suites for MQTT to mqtt.tesaiot.com:8884
 * Server uses RSA cert with ECDHE-RSA-AES256-GCM-SHA384.
 * Dependencies: MBEDTLS_ECDH_C, MBEDTLS_RSA_C, MBEDTLS_PKCS1_V15. */
 ...
 */
/* Required by MBEDTLS_PSA_CRYPTO_SE_C (check_config.h prerequisite).
 * Reference pse84_tesaiot_client leaves the platform default (defined).
 * ITS backend: psa_its_ram_stubs.c (volatile, RAM-only). */
 ...
 */
/* TLS 1.3 requires MBEDTLS_PSA_CRYPTO_C which is disabled (WiFi conflict).
 * Server supports TLS 1.2 (ECDHE-RSA-AES256-GCM-SHA384).
 * Must #undef — base mbedtls_config.h may define TLS 1.3. */
 ...
 */
/* Downgraded from REMOVED to WARNING to allow MBEDTLS_PSA_CRYPTO_SE_C
 * (OPTIGA Trust M PSA SE driver). SE_C is deprecated in mbedTLS 3.6
 * but required for hardware key delegation to OPTIGA. */
...
/* IFX_PSA_MXCRYPTO_PRESENT — NOT defined.
 * CY_CRYPTO_HAL_DISABLE removes the HW crypto HAL, so telling PSA that
 * hardware acceleration exists causes dispatch to a removed driver → TLS fail.
 * Reference pse84_tesaiot_client omits these two defines. */
/* #undef IFX_PSA_MXCRYPTO_PRESENT */
/* #undef PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
#define MBEDTLS_PSA_CRYPTO_DRIVERS
/* Enable PSA Secure Element driver (OPTIGA Trust M mTLS key delegation) */
 ...
 */
/* DISABLED: psa_crypto_init() is called in cy_tls_create_context() but
 * cy_tls_load_global_root_ca_certificates() parses the root CA BEFORE that.
 * With USE_PSA_CRYPTO, x509_crt_parse uses PSA hashes → fails because PSA
 * is not yet initialized → error 0x08260007 (TLS_PARSE_CERTIFICATE).
 * Phase G: Re-enabled for mTLS + OPTIGA Trust M.
 * mbedtls_pk_setup_opaque() requires USE_PSA_CRYPTO to bind OPTIGA key to TLS.
 * IMPORTANT: psa_crypto_init() MUST be called at boot (in main.c) BEFORE any
 * TLS operations, otherwise x509_crt_parse fails with PSA hash not initialized. */
...
/******************************************************************************
 * TLS I/O buffer sizing — critical for RAM-constrained devices.
 *
 * Defaults: 16384 in + 16384 out = 32KB heap during TLS handshake.
 * MQTT over TLS 1.2 needs:
 *   IN:  ~6KB (ServerCertificate chain is the largest incoming record)
 *   OUT: ~2KB (ClientHello + ClientKeyExchange are small)
 * Saving: (16384-6144) + (16384-2048) = 24,576 bytes freed.
 *
 * If TLS handshake fails with MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL,
 * increase MBEDTLS_SSL_IN_CONTENT_LEN first (server cert chain size).
 *****************************************************************************/
