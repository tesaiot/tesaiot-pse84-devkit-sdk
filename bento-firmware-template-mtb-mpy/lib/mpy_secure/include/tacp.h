...
/* Firmware version query (AA 55 05) — responds with build info */
#define TACP_CMD_VERSION_QUERY  0x05    /* Query firmware build info          */

/* Hard reset (AA 55 06) — NVIC_SystemReset(), all cores restart */
#define TACP_CMD_HARD_RESET     0x06    /* Force hardware reset immediately   */

/* Binary file transfer (AA 55 20 <frame>) */
#define TACP_CMD_FILE_XFER      0x20    /* Start binary file upload           */

/* BentoClaw agent commands (AA 55 30 <sub-protocol>) */
#define TACP_CMD_BENTOCLAW      0x30    /* BentoClaw binary sub-protocol      */

/* BentoClaw sub-commands (byte after 0x30) — v2 protocol:
 *   Request frame  (IDE → Device):
 *     [0xAA][0x55][0x30][sub_cmd][seq][len_lo][len_hi][payload][crc_lo][crc_hi]
 *
 *   Response frame (Device → IDE):
 *     [0xAA][0x55][0x30][sub_cmd][seq][len_lo][len_hi][payload][crc_lo][crc_hi]
 *     sub_cmd matches request for success; 0xFF for errors.
 *     seq echoes the request's sequence number (0 for unsolicited like READY).
 *
 *   CRC v2: CRC-16/CCITT (init 0xFFFF, poly 0x1021) over:
 *           sub_cmd + seq + len_lo + len_hi + payload
 *
 *   Version negotiation: READY frame includes {"proto":2} — IDE auto-selects.
 */
#define TACP_CLAW_READY         0x00    /* FW→IDE: boot complete, ready       */
#define TACP_CLAW_EXEC          0x01    /* Execute tool                       */
#define TACP_CLAW_ASK           0x02    /* Send prompt to AI backend          */
#define TACP_CLAW_STATUS        0x03    /* Get agent status                   */
#define TACP_CLAW_TOOLS         0x04    /* List available tools               */
#define TACP_CLAW_HISTORY       0x05    /* Get session history                */
#define TACP_CLAW_REMEMBER      0x06    /* Store memory key=value             */
#define TACP_CLAW_CLEAR         0x07    /* Clear session                      */
#define TACP_CLAW_CONNECT       0x08    /* Connect to HTTPS backend           */
#define TACP_CLAW_DISCONNECT    0x09    /* Disconnect from backend            */
#define TACP_CLAW_ERROR         0xFF    /* Error response (payload=message)   */

#define TACP_CLAW_PAYLOAD_MAX   (3072)  /* Max payload size for sub-protocol  */

/*******************************************************************************
 * Board SKU Registry
 * Format: KIT_<chip>_<variant>_<revision>
 * ─────────────────────────────────────────────
 * KIT_PSE84_AI_001         AI Kit Sensor Hub
 * KIT_PSE84_AI_002         AI Kit Game Console
 * KIT_PSE84_EVAL_EPC2_001  Eva Kit Sensor Hub
 * ─────────────────────────────────────────────
 * Add new boards by incrementing revision number.
 * SKU is defined in mpconfigboard.h (default) or overridden
 * via -DMICROPY_HW_BOARD_SKU in per-project Makefile.micropython.
 ******************************************************************************/
...
/** ISR-safe trigger for the CM55 "Delete main.py" button. Breaks a running /
 *  looping script (keyboard interrupt) and forces a MicroPython soft reset so
 *  the boot path removes /main.py. Safe to call from interrupt context. */
...
/** Send a BentoClaw TACP binary response frame.
 *  Frame: [0xAA][0x55][0x30][sub_cmd][len_lo][len_hi][payload][crc_lo][crc_hi]
 *  Use sub_cmd matching request for success, TACP_CLAW_ERROR (0xFF) for errors. */
