...
/**
 * Forget every slot, including slot 0.
 *
 * Only correct after PSA itself has forgotten the keys. mbedtls_psa_crypto_free()
 * — which cy_tls_deinit() calls when the MQTT client is torn down — wipes the
 * volatile key store and unregisters secure-element drivers, but it never calls
 * p_destroy, so this map is left describing keys that no longer exist. The next
 * allocation then skips the slots it believes are taken and works its way up:
 * slot 0, then 1, then 2, then 3, and the fourth reconnection of a boot fails
 * with PSA_ERROR_INSUFFICIENT_MEMORY. Observed 2026-08-05, slot 0 on the first
 * connection and slot 1 after a disconnect.
 *
 * This is deliberately not optiga_psa_clear_all_slots(), which preserves slot 0
 * so a cached mbedTLS key handle keeps working. That reasoning is right while
 * the handle is alive and wrong once PSA has dropped it — the two cases need
 * different answers, so they get different functions.
 *
 * Nothing is destroyed in the secure element: these entries only name keys that
 * live in the chip.
 */
void optiga_psa_forget_all_slots(void)
{
    ...
    printf("[PSA-SE] Slot map cleared — PSA no longer knows these keys\n");
}
		...
		size_t sig_size, size_t *sig_len)
{
	...
	/* Any ECDSA variant — the hash is not ours to choose.
	 *
	 * TLS 1.2 derives the CertificateVerify hash from the negotiated
	 * ciphersuite's PRF rather than from the key's curve, so a P-256 key is
	 * routinely asked to sign a SHA-384 digest. Refusing that stalls the
	 * handshake; ECDSA is defined for it. */
	if (!PSA_ALG_IS_ECDSA(alg)) {
		printf("[PSA-Sign] ERROR: alg 0x%08lX is not ECDSA\n", (unsigned long)alg);
		return PSA_ERROR_NOT_SUPPORTED;
	}

	/* Use the leftmost 256 bits when the digest is longer than the group order.
	 *
	 * SEC1 2.0 §4.1.3 and FIPS 186-4 §6.4 both define the ECDSA message
	 * representative as the leftmost min(N, outlen) bits of the digest, where N
	 * is the bit length of n. A verifier holding the same P-256 key performs the
	 * identical truncation, so signing SHA-384 this way interoperates exactly.
	 * The built-in mbedTLS ECDSA does this inside derive_mpi(); a PSA secure-
	 * element driver receives the untruncated digest and must do it itself. */
	size_t optiga_hash_len = hash_length;
	if (optiga_hash_len > 32) {
		optiga_hash_len = 32;
	}

	if (optiga_hash_len < 32) {
		printf("[PSA-Sign] ERROR: digest %u bytes is shorter than the P-256 order\n",
			   (unsigned)hash_length);
		return PSA_ERROR_INVALID_ARGUMENT;
	}

	if (sig_size < 64) {
		printf("[PSA-Sign] ERROR: sig_size %u < 64\n", (unsigned)sig_size);
		return PSA_ERROR_BUFFER_TOO_SMALL;
	}
	...
	optiga_lib_status_t status = trustm_ecdsa_sign(map->oid,
			hash, (uint16_t) optiga_hash_len, sig, &out);

	if (status != OPTIGA_LIB_SUCCESS) {
		printf("[PSA-Sign] ERROR: trustm_ecdsa_sign status=0x%04X\n",
			   (unsigned)status);
		return PSA_ERROR_HARDWARE_FAILURE;
	}
