...
/* PDM gain, and the scale runs backwards.
 *
 * Cy_PDM_PCM_SetGain takes an enum where 0 is CY_PDM_PCM_SEL_GAIN_83DB -- the
 * LOUDEST setting, 83 dB -- and larger numbers are quieter, down to -67 dB at
 * 25. The default here was 0, so every microphone read on this port ran at
 * maximum gain and came back pinned at the rail: measured on an Eva Kit
 * 2026-08-14, rms sat at 87% of full scale in a quiet room and did not move
 * when the board played tones at its own speaker.
 *
 * 13 is CY_PDM_PCM_SEL_GAIN_5DB, which is what CM55's audio_pdm.c uses for the
 * DEEPCRAFT audio models on this same microphone, and those models hear real
 * speech. */
#define DEFAULT_LEFT_GAIN                   13
#define DEFAULT_RIGHT_GAIN                  13
...
/* rxFifoTriggerLevel is written as 31 into the channel config, and the trigger
 * asserts when MORE than that many words are present -- so the ISR runs with
 * 32 words waiting and must drain 32. Draining 31 left one word behind on
 * every single interrupt: the FIFO crept up, overflowed, and from then on the
 * stream was permanently out of phase. It still looked like a continuous
 * waveform, which is why it survived several rounds of chasing the clock, the
 * channel and the sample delay. CM55's own driver says this in a comment and
 * drains HW_FIFO_SIZE/2 = 32.
 *
 * 1024 / 32 = 32 exactly, so a frame is now a whole number of interrupts too;
 * with 31 it was 33 interrupts covering 1023 samples and the buffer boundary
 * slid by one sample per frame. */
#define RX_FIFO_TRIG_LEVEL              (31)
#define RX_FIFO_DRAIN_WORDS             (RX_FIFO_TRIG_LEVEL + 1)
#define NUMBER_INTERRUPTS_FOR_FRAME     ((FRAME_SIZE) / RX_FIFO_DRAIN_WORDS)
